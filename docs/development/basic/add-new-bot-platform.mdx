...
description: >-
  Learn how to add a new bot platform (e.g., Slack, WhatsApp) to Ethos's
...
This guide walks through the steps to add a new bot platform to Ethos's channel system. The platform architecture is modular — each platform is a self-contained directory under `src/server/services/bot/platforms/`.
...
### Option B: Develop a Python backend platform adapter

If no npm adapter exists, add the platform under `python-backend/app/services/bot/platforms/`.
Feishu/Lark, LINE, QQ, and WeChat have been migrated to the Python backend and should be used as the reference implementations.

Each Python platform follows this structure:

```
python-backend/app/services/bot/platforms/<platform>/
├── __init__.py
├── client.py      # Platform API client
├── definition.py  # Platform metadata and schema
└── ...
...
Follow the structure of existing docs (e.g., `discord.mdx`): Prerequisites → Create App → Configure in Ethos → Configure Webhooks → Test Connection → Configuration Reference → Troubleshooting.
