...
description: >-
  Explore the client-server interaction logic of Ethos Chat API, including
...
Ethos Chat API in client-server interactions,
...
Ethos supports three types of tools:
...
- **Server-side loop** (queue/local):
  ...
  API/Cron triggers. The loop runs in the Python backend,
  streaming events to the client via FastAPI SSE endpoints under
  `/api/ai-agent/*` and `/api/agent/*`.

## Model Runtime

Model Runtime (`packages/model-runtime/`) is the core
abstraction layer in Ethos for interacting with
...
Agent Runtime (`packages/agent-runtime/`) is Ethos's
