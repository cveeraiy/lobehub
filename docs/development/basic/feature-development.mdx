---
title: Ethos Feature Development Complete Guide
description: A comprehensive guide for developers on implementing features in Ethos.
tags:
  - Ethos
  - Feature Development
  - Developer Guide
...
# Ethos Feature Development Complete Guide

This document aims to guide developers on how to develop a complete feature in Ethos.
...
Ethos uses a PostgreSQL database, with [Drizzle ORM](https://orm.drizzle.team/) to operate the database.

All schemas are located in `src/database/schemas/`. We need to adjust the `agents` table to add two fields corresponding to the configuration items:
...
| Repository        | src/database/repositories/  | Complex queries, cross-table operations               |
| DB Model          | src/database/models/        | Single-table CRUD operations                          |
+-------------------+--------------------------------------+------------------------------------------------------+
```
...
Ethos uses [zustand](https://zustand.docs.pmnd.rs/getting-started/introduction) as the global state management framework. For detailed practices on state management, refer to [State Management Best Practices](/docs/development/state-management/state-management-intro).
...
Ethos is an internationalized project using [react-i18next](https://github.com/i18next/react-i18next). Newly added UI text needs to:
...
The above is the complete implementation process for the Ethos opening settings feature, covering the full chain from database schema → data model → Service/Model → Store → i18n → UI → testing. Developers can refer to this document for developing related features.
