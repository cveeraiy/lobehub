---
title: Ethos Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for Ethos Vercel deployments and
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest Ethos image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running Ethos Container

  Assuming the Ethos container is named `lobehub`, use the following commands to stop and remove the currently running Ethos container:
  ...
  ### Pull the Latest Ethos Image

  Use the following command to pull the latest Docker image for Ethos:
  ...
  Redeploy the Ethos container using the newly pulled image:
...
<Callout type={'tip'}>
  **If I redeploy, will I lose my local chat records?**
  ...
  No need to worry, you won't. All of Ethos's chat records are stored in your local browser. Therefore, when redeploying Ethos using Docker, your chat records will not be lost.
</Callout>
