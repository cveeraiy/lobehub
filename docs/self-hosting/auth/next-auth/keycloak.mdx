---
title: Configuring Keycloak Authentication Service in Ethos
description: >-
  Learn how to configure the Keycloak authentication service in Ethos,
...
# Configuring Keycloak Authentication Service in Ethos
...
<Callout type={'tip'}>
  If you want to privately deploy Keycloak, we recommend using it together with Ethos via Docker
...
- Your Ethos database version IP/port is `http://LobeHub_IP:3210`.
- Your privately deployed Keycloak domain is `http://KEYCLOAK_IP:8080`.

If you deploy using a public network, this guide assumes:

- Your Ethos database version domain is `https://lobe.example.com`.
...
<Steps>
  ...
  1. Create a new Realm
     ...
     - Enter a name, such as "Ethos", then click "Create"
  ...
  2. Create a Client
     ...
     - On the "Login settings" page:
       ...
       - Web origins: Add your Ethos domain or IP
       - Click "Save"
  ...
  Set the obtained client ID and client secret as `AUTH_KEYCLOAK_ID` and `AUTH_KEYCLOAK_SECRET` in the Ethos environment variables.

  Configure the Ethos environment variable `AUTH_KEYCLOAK_ISSUER` as:

  - `http://localhost:8080/realms/Ethos` for local development environment
  - `http://KEYCLOAK_IP:8080/realms/Ethos` for privately deployed Keycloak on a local network
  - `https://lobe-auth-api.example.com/realms/Ethos` for Keycloak deployed in a public environment
  ...
  | `NEXT_AUTH_SSO_PROVIDERS` | Required | Select the single sign-on provider for Ethos. For Keycloak, fill in `keycloak`.                                                                                                          |
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate through Keycloak and use Ethos.
</Callout>
