...
| `@lobechat/agent-manager-runtime`            | `removed`          | `removed`                      | Standalone package removed; remaining TS agent manager helper lives in `@lobechat/builtin-tools` for agent builder/management executors backed by agent/AI-agent service flows.                       |
...
| `@lobechat/builtin-tool-activator`           | `removed`          | `removed`                      | Standalone package removed; TS manifest/render/inspector/executor/runtime contract now lives in `@lobechat/builtin-tools`, with skill activation delegated to Python tools.                           |
| `@lobechat/builtin-tool-agent-builder`       | `removed`          | `removed`                      | Standalone package removed; remaining TS manifest/client/executor contract lives in `@lobechat/builtin-tools`, backed by agent/AI-agent service flows.                                                |
| `@lobechat/builtin-tool-agent-documents`     | `removed`          | `removed`                      | Python `agent_documents_tool.py` owns `lobe-agent-documents` runtime execution through `/api/tools/run`; remaining TS manifest/render/executor contract lives in `@lobechat/builtin-tools`.           |
| `@lobechat/builtin-tool-agent-management`    | `removed`          | `removed`                      | Standalone package removed; remaining TS manifest/render/streaming/executor contract lives in `@lobechat/builtin-tools`, backed by agent and AI-agent services.                                       |
| `@lobechat/builtin-tool-agent-marketplace`   | `removed`          | `removed`                      | Standalone package removed; UI intervention/runtime picker contract now lives in `@lobechat/builtin-tools`, while marketplace data continues through REST services.                                   |
...
| `@lobechat/builtin-tool-cloud-sandbox`       | `removed`          | `removed`                      | Standalone package removed; remaining TS sandbox manifest/client/executor/runtime contract lives in `@lobechat/builtin-tools`, backed by sandbox services.                                            |
| `@lobechat/builtin-tool-creds`               | `removed`          | `removed`                      | Standalone package removed; remaining TS creds manifest/context/executor/runtime contract lives in `@lobechat/builtin-tools`, backed by key vault and Klavis service flows.                           |
...
| `@lobechat/builtin-tool-group-agent-builder` | `removed`          | `removed`                      | Standalone package removed; remaining TS group-agent-builder manifest/client/executor contract lives in `@lobechat/builtin-tools`, backed by chat group and agent service flows.                      |
| `@lobechat/builtin-tool-group-management`    | `removed`          | `removed`                      | Standalone package removed; remaining TS group orchestration manifest/client/executor contract lives in `@lobechat/builtin-tools`, backed by group/task services.                                     |
| `@lobechat/builtin-tool-gtd`                 | `removed`          | `removed`                      | Standalone package removed; remaining TS GTD manifest/client/executor/runtime contract lives in `@lobechat/builtin-tools`, backed by task/document services.                                          |
| `@lobechat/builtin-tool-knowledge-base`      | `removed`          | `removed`                      | Standalone package removed; remaining TS manifest/render/inspector/executor/runtime contract lives in `@lobechat/builtin-tools`, backed by Python knowledge/files/chunks services.                    |
...
| `@lobechat/builtin-tool-memory`              | `removed`          | `removed`                      | Python `memory_tool.py` owns `lobe-user-memory` runtime execution through `/api/tools/run`, including TS-parity search formatting, time-intent normalization, and runtime write guards.               |
| `@lobechat/builtin-tool-message`             | `removed`          | `removed`                      | Standalone package removed; remaining TS manifest/render/inspector/intervention/executor/runtime contract lives in `@lobechat/builtin-tools`, backed by bot/message services.                         |
| `@lobechat/builtin-tool-notebook`            | `removed`          | `removed`                      | Python `notebook_tool.py` owns legacy `lobe-notebook` document runtime execution through `/api/tools/run`; remaining TS manifest/render/executor contract lives in `@lobechat/builtin-tools`.         |
...
| `@lobechat/builtin-tool-remote-device`       | `removed`          | `removed`                      | Standalone package removed; remaining TS remote-device manifest/runtime contract lives in `@lobechat/builtin-tools`. Device execution remains TS/device-owned rather than Python-only.                |
...
| `@lobechat/builtin-tool-skill-store`         | `removed`          | `removed`                      | Python has skill/market services and `skill_store.py`. The remaining TS manifest/render/inspector wrapper now lives in `@lobechat/builtin-tools`.                                                     |
| `@lobechat/builtin-tool-skills`              | `removed`          | `removed`                      | Python has skill engine and `skills.py`. TS now keeps only manifest/render/inspector and a thin REST executor inside `@lobechat/builtin-tools`.                                                       |
...
| `@lobechat/builtin-tool-user-interaction`    | `removed`          | `removed`                      | Standalone package removed; remaining TS human-interaction manifest/client/executor/runtime contract lives in `@lobechat/builtin-tools`, backed by AI-agent intervention flows.                       |
| `@lobechat/builtin-tool-web-browsing`        | `removed`          | `removed`                      | Standalone package removed; remaining TS web-browsing manifest/client/runtime contract lives in `@lobechat/builtin-tools`, backed by web search/crawler service flows.                                |
...
| `@lobechat/database`                         | `removed`          | `moved-to-src-compat`          | Workspace package removed. Remaining legacy TypeScript Drizzle code lives under `src/database` only as a compatibility island while Python SQLModel owns database behavior.                           |
...
| `@lobechat/file-loaders`                     | `removed`          | `removed`                      | Standalone package removed; local desktop parsing helpers now live under `@lobechat/local-file-shell`, while Python owns REST file/chunk/RAG parsing paths.                                           |
...
| `@lobechat/markdown-patch`                   | `removed`          | `removed`                      | Package removed; Python onboarding PATCH now owns markdown patch application semantics, while the TS hunk request type lives in `@lobechat/builtin-tools`.                                            |
| `@lobechat/memory-user-memory`               | `removed`          | `removed`                      | Package removed; memory tool request schemas now live in `@lobechat/types`, while Python memory routers/services/workflows own memory execution and extraction behavior.                              |
...
| `@lobechat/web-crawler`                      | `removed`          | `removed`                      | Package removed; Python web search/crawler services own crawling behavior, while shared crawler response shapes now live in `@lobechat/types`.                                                        |
