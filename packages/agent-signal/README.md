...
Do not add app integration here. Side effects belong in the Python backend Agent Signal service or the browser service facade.
...
Use the Python backend Agent Signal service for:
...
See `python-backend/app/routers/agent_signal.py` and `python-backend/app/services/agent_signal/`.

## Supported Source Events

| Source event                   | Constant                                              | Primary producer                                                                           | Payload intent                                                                                            |
| ------------------------------ | ----------------------------------------------------- | ------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| `agent.execution.completed`    | `AGENT_SIGNAL_SOURCE_TYPES.agentExecutionCompleted`   | Python agent runtime                                                                       | Server agent execution completed with operation, step, topic, and context metadata.                       |
| `agent.execution.failed`       | `AGENT_SIGNAL_SOURCE_TYPES.agentExecutionFailed`      | Python agent runtime                                                                       | Server agent execution failed with operation, reason, error, topic, and context metadata.                 |
| `agent.user.message`           | `AGENT_SIGNAL_SOURCE_TYPES.agentUserMessage`          | Python workflow bridge, bot router                                                         | User feedback/message content that policies can analyze for memory, prompt, document, or skill changes.   |
...
| `runtime.after_step`           | `AGENT_SIGNAL_SOURCE_TYPES.runtimeAfterStep`          | Python agent runtime                                                                       | Server runtime step finished with operation, step index, topic, and context metadata.                     |
| `runtime.before_step`          | `AGENT_SIGNAL_SOURCE_TYPES.runtimeBeforeStep`         | Python agent runtime                                                                       | Server runtime step is about to run with operation, step index, topic, and context metadata.              |

`AGENT_SIGNAL_CLIENT_SOURCE_TYPES` is the allow-list for browser-originated events accepted by the Python Agent Signal API. It currently includes only the `client.*` source events.
...
- [Python Agent Signal API](../../python-backend/app/routers/agent_signal.py): authenticated ingress.
- [Python orchestrator](../../python-backend/app/services/agent_signal/orchestrator.py): source event execution through registered policies.
- [Python agent runtime](../../python-backend/app/services/agent_runtime/): server runtime source event producers.
...
2. Add Python normalization in `python-backend/app/services/agent_signal/` if the generic payload needs normalization before becoming an `AgentSignalSource`.
3. Register the Python handler/policy in `python-backend/app/services/agent_signal/orchestrator.py`.
