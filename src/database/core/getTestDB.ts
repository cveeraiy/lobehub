...
export const getTestDB = async (): Promise<LobeChatDatabase> => {
  ...
  // Historical migration 0050_add_user_organization.sql is not present in the
  // Drizzle journal, so readMigrationFiles() does not apply it for PGlite.
  await testClientDB.execute(sql`ALTER TABLE "users" ADD COLUMN IF NOT EXISTS "organization" text`);
