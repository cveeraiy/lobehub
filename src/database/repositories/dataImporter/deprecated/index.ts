  ...
  importData = async (data: ImporterEntryData) => {
    ...
    await this.db.transaction(async (trx) => {
      ...
      if (data.messages && data.messages.length > 0) {
        ...
        console.info(`find messages: ${Date.now() - findStart}ms`);
        ...
        if (shouldInsertMessages.length > 0) {
          ...
          console.info(`insert messages: ${Date.now() - insertStart}ms`);
          ...
          console.info(`execute updates parentId: ${Date.now() - parentIdStart}ms`);
