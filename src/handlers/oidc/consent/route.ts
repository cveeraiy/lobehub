...
export async function POST(request: Request) {
  ...
  try {
    ...
    let details;
    try {
      details = await oidcService.getInteractionDetails(uid, request);
      log(
    ...
    const internalRedirectUrlString = await oidcService.getInteractionResult(uid, result, request);
    ...
    if (appEnv.APP_URL) {
      ...
      return new Response(null, {
        headers: { Location: finalRedirectUrl.toString() },
        status: 303,
      });
    }
    ...
    return new Response(null, {
      headers: { Location: internalRedirectUrlString },
      status: 303,
    });
  } catch (error) {
