...
class AgentService {
  ...
  getFilesAndKnowledgeBases = async (agentId: string) => {
    return restClient.get(`/agents/${agentId}/knowledge-and-files`);
  };

  getAgentConfigById = async (agentId: string) => {
    return toAgentItem(await restClient.get<RawAgentItem | null>(`/agents/${agentId}`));
  };

  /** @deprecated use getAgentConfigById instead */
  getSessionConfig = async (sessionId: string) => {
    return restClient.get(`/agents/config-by-session/${sessionId}`);
  };
