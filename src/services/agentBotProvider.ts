...
class AgentBotProviderService {
  ...
  wechatGetQrCode = async (): Promise<{ qrcode: string; qrcode_img_content: string }> => {
    return restClient.post('/agent-bot-providers/wechat/qrcode', {});
  };

  wechatPollQrStatus = async (
    qrcode: string,
  ): Promise<{
    baseurl?: string;
    bot_token?: string;
    ...
    return restClient.post('/agent-bot-providers/wechat/qrcode/status', {
      body: { qrcode },
    });
  };
}
