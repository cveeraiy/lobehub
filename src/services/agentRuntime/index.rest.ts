/**
 * Agent Runtime service — REST API version.
 *
 * Drop-in replacement for `src/services/agentRuntime/index.ts` (TRPC version).
 * Calls the Python backend's `/api/ai-agent/*` REST endpoints directly.
 *
 * Note: `createOperation` does significant client-side work (agent store reads,
 * tool generation, context engineering) before making the REST call — identical
 * to the TRPC version except the final network call uses restClient.
 */
